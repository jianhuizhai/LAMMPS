...
ylayers = round( (np.max(data[:,1]) - np.min(data[:,1]))/YDistance )       # 向下取整
zlayers = round( (np.max(data[:,2]) - np.min(data[:,2]))/ZDistance ) 

print('np.max(data[:,2] =', np.max(data[:,2]))
print('np.min(data[:,2])=', np.min(data[:,2]))
print('(np.max(data[:,2]) - np.min(data[:,2]))/ZDistance=',(np.max(data[:,2]) - np.min(data[:,2]))/ZDistance)
print('round(=======================================)   =', round((np.max(data[:,2]) - np.min(data[:,2]))/ZDistance))
ylayers = int(ylayers)
zlayers = int(zlayers)
#===========================================================================
...
#========================================================================
        ...
        #====================================================================================
        atoms = int( (len_above + len_below - 2 )*0.5 )
        
        # print("atoms = ",atoms)
        disregistry = np.zeros(atoms)
        if(len(half2)> len(half1)):
            atoms_above  = np.array( [x[0] for x in half2] )[0:len(half2)-2]  # x[0] command is used to get first column value of list half2
            atoms_below  = np.array( [x[0] for x in half1] )
            atoms_disreg = atoms_below
        else:
            atoms_above  = np.array( [x[0] for x in half2] )
            atoms_below  = np.array( [x[0] for x in half1] )[0:len(half1)-2]
            atoms_disreg = atoms_above

        disregistry = -(atoms_above - atoms_below)
